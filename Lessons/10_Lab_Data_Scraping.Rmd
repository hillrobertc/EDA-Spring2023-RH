...
#Load in data using the package's "readNWISdv" function. This package is specifically for pulling water quality data from the USGS NWIS
...
```{r}
#Apply the API key (this is Jon's key)
census_api_key("d4ad7874c996c7a83fec0a81466309ddc07d8e22", install = TRUE)

nc_pop <- 
  get_acs(geography = "county",
          variables = "B01003_001", #this variable is population
          state = "NC",
          geometry = TRUE)
...
the_states <- html_node(the_website,"td:nth-child(1)") %>% html_text()
the_price <- html_node(the_website,"td:nth-child(2)") %>% html_text()
the_capacity <- html_node(the_website,"td:nth-child(3)") %>% html_text() 
net_generation <- html_node(the_website,"td:nth-child(4)") %>% html_text()
total_retail <- html_node(the_website,"td:nth-child(5)") %>% html_text()
