...
O3.violin <-
  ...
  stat_summary(geom="text",fun=max,vjust=-1,size=4,label=c("a","b")) +
  labs(fill="",y="Ozone AQI values", x="")
print(O3.violin)
...
EPAair_corr <-EPAair %>%
  ...
  na.omit()
corrplot.mixed(cor(EPAair_corr),upper="ellipse")
...
OzonebyALL.regression <-
  lm(data=EPAair_corr,PM2.5 ~ Ozone + Year + Month + mean_Lng + mean_Lat)
...
OzoneBest.regression <-
  lm(data=EPAair_corr,PM2.5 ~ Ozone + Month + mean_Lng + mean_Lat)
