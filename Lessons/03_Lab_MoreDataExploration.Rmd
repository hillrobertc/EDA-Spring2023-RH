...
-   **Lists** Oadrdered collection of elements that can have different modes.
...
```{r DataExercise-1a}
weatherhigh <- c(54, 71, 72, 63, 51, 52, 61, 64, 57, 73)
weatherlow <- c(42, 49, 60, 46, 32, 32, 41, 47, 52, 54)
weatherdate <- c(23027, 230208,230209, 230210, 230211, 230212, 230213, 230214, 230215, 230216)
```
...
highC <- convert(weatherhigh)
lowC <- convert(weatherlow)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
df_Temps <- data.frame(weatherhigh, weatherlow, highC, lowC)
```
...
#sd works on vectors but not data frames
```
